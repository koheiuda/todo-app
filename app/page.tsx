  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">SEOニュース</h1>
        ...
        <Card className="border-red-300 bg-red-50 dark:bg-red-950/30">
          <CardContent className="pt-6 text-sm">
            ...
            <span className="text-zinc-500">
              .env.local の POSTGRES_URL を設定し、`npm run db:push` で初期化してください。
            </span>
          </CardContent>
        </Card>
      )}
      ...
      {list.length === 0 && !dbError && (
        <Card>
          <CardContent className="pt-6 text-sm text-zinc-500">
            まだ記事がありません。手動取得：
            <code className="ml-2 px-2 py-1 rounded bg-zinc-100 dark:bg-zinc-800">
            ...
            <Card className="hover:border-zinc-400 transition-colors">
              <CardHeader>
                <div className="flex items-center gap-2 text-xs text-zinc-500">
                  ...
                  {a.publishedAt && <span>公開: {formatJst(a.publishedAt)}</span>}
                </div>
